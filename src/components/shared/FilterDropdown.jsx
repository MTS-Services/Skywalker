  ...
  return (
    <div className="relative w-full sm:w-auto" ref={dropdownRef}>
      ...
      >
        ...
        <span className={`text-primary-400 transition-transform ${isOpen ? "rotate-180" : ""}`}>
          <svg
        ...
        <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-primary-200 bg-white shadow-lg">
          <div className="p-2">
            <input
              ...
              className={`w-full rounded-lg border border-primary-100 focus:outline-none focus:ring-1 focus:ring-primary-400 p-2 ${isRTL ? "text-right" : "text-left"}`}
              value={searchTerm}
              ...
              filteredOptions.map((option) => (
                ...
                >
                  <div className={`flex items-center ${isRTL ? "text-right" : "text-left"}`}>
                    ...
                    />
                    <span className={`text-primary-900 ${isRTL ? "mr-2" : "ml-2"}`}>{option.name}</span>
                  </div>
                  {option.count && <span className="text-sm text-primary-900">({option.count})</span>}
                </li>
              ))
            ) : (
              <li className="p-2 text-center text-primary-900">No results found</li>
            )}
          </ul>
        </div>
      )}
        ...
        >
          ...
          <ul className="p-1">
            {filteredOptions.length > 0 ? (
              ...
              <li className="p-2 text-center text-gray-500">{searchPlaceholder || "No results"}</li>
            )}
          </ul>
        </div>
      )}
