...
function Header() {
  ...
  const desktopNavItems = useMemo(() => {
    const base = [{ label: "Home (Search)", to: "/" }];
    const auth = [
    ...
    const protectedItems = [
      ...
      { label: "My Archives", to: "/my-archives" },
      { label: "Buy Credits", to: "/buy-credits" },
      { label: "Logout", action: handleLogout },
    ];
    const end = [{ label: "Agents", to: "/agents" }];
  ...
  return (
    <nav
      className={`border-primary-500 relative z-50 border-b bg-white px-4 py-4 shadow-sm ${isRTL ? "rtl" : "ltr"
        }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <NavLink
          to={isAuthenticated ? "/add-upload" : "/login"}
          className={`flex items-center gap-2 bg-primary-300/10 px-5 py-2 rounded-md border border-primary-300/40`}
        >
          <FaPlusCircle className={`text-primary-600 text-lg ${isRTL ? "ml-1" : "mr-1"}`} />
          {t.header.addFreeAd}
        </NavLink>
      </div>
