...
const SearchFilterBar = ({ initialFilters, t, isRTL }) => {
  ...
  return (
    <div className="w-full space-y-4 rounded-2xl bg-white p-4 shadow-lg sm:p-6">
      ...
      <div>
        {/* all of is a dropdown... like select field with responsive*/}

        {/* there will be four dropdowns */}
        {/* 1st move transaction type here on select it will be search from json file. default select first item if not selected.*/}
        {/* 2nd move property type here on select it will be search from json file. default select first item if not selected.*/}
        {/* 3rd make Price filter Min and Max here with slider range also have custom input filed with reset button and submit button, on submit it will be search from json file*/}
        {/* 4th search by text here with search field and reset and submit button, */}
        ...
        <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-primary-200 bg-white shadow-lg">
          <div className="p-2">
            <input
              ...
              className={`w-full rounded-lg border border-primary-100  p-2 ${isRTL ? "text-right" : "text-left"}`}
              value={searchTerm}
